defmodule CryptoScannerWeb.ScannerChannel do
  ...
  @initial_filter %{"period" => "5m", "percentage" => -10, "volume" => 1}
  ...
  def handle_out("tick_alert", %{"coins" => coins, "btc_price" => btc_price, "eth_price" => eth_price}, socket) do
    ...
    if coins != [] do
      data = %{coins: calc_hot(coins, period, percentage, volume, btc_price, eth_price)}

      count = Enum.count(data.coins)

      Logger.info(">>>>> Scanner Tick Filter: #{inspect(filter)} >> Results:  #{count} <<<<<")
      push socket, "tick_alert", data
    else
      Logger.info(">>>>> Scanner Tick Filter - Empty Coins")
    end
